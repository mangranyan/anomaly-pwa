...
async function renderForm() {
  ...
  return `
    <div class="p-4 pt-6 max-w-md mx-auto">
      ...
      <div class="space-y-4">
        <div>
          <label class="block text-gray-700 mb-1">提报人 <span class="text-red-500">*</span></label>
          <input type="text" id="reporter" class="w-full h-14 px-4 border border-gray-300 rounded-lg text-base" placeholder="请输入姓名" required>
        </div>

        <div>
          <label class="block text-gray-700 mb-1">异常描述 <span class="text-red-500">*</span></label>
          <textarea id="desc" class="w-full h-24 px-4 py-2 border border-gray-300 rounded-lg text-base" placeholder="请详细描述异常现象" required></textarea>
        </div>

        ${!isDevice ? `
          <div>
            <label class="block text-gray-700 mb-1">良率（如：95%）</label>
            <input type="text" id="yield" class="w-full h-14 px-4 border border-gray-300 rounded-lg text-base" placeholder="例：95%">
          </div>
          <div>
            <label class="block text-gray-700 mb-1">临时处置措施</label>
            <textarea id="measure" class="w-full h-20 px-4 py-2 border border-gray-300 rounded-lg text-base" placeholder="例：暂停该工位作业"></textarea>
          </div>
        ` : ''}
    ...
    <script>
      document.getElementById('submit-btn').onclick = async () => {
        ...
        const data = {
          product: '${escapeHtml(currentParams.product)}',
          line: '${escapeHtml(currentParams.line)}',
          workstation: '${escapeHtml(currentParams.station)}',
          deviceName: isDevice ? '${escapeHtml(currentParams.deviceName || '')}' : '',
          assetCode: isDevice ? '${escapeHtml(currentParams.assetCode || '')}' : '',
          exception_type: '${escapeHtml(currentParams.type)}',
  ...
  return `
    ...
    <script>
      ...
      function renderFields() {
        ...
        container.innerHTML = \`
          <div>
            <label class="block text-gray-700 mb-1">产品</label>
            <input type="text" id="product" class="w-full h-14 px-4 border border-gray-300 rounded-lg" placeholder="例：TWS耳机">
          </div>
          <div>
            <label class="block text-gray-700 mb-1">线别</label>
            <input type="text" id="line" class="w-full h-14 px-4 border border-gray-300 rounded-lg" placeholder="例：SMT-01">
          </div>
          <div>
            <label class="block text-gray-700 mb-1">工站</label>
            <input type="text" id="station" class="w-full h-14 px-4 border border-gray-300 rounded-lg" placeholder="例：AOI检测">
          </div>
          \${currentType === 'device' ? \`
            <div>
              <label class="block text-gray-700 mb-1">设备名称</label>
              <input type="text" id="deviceName" class="w-full h-14 px-4 border border-gray-300 rounded-lg" placeholder="例：回流焊机">
            </div>
            <div>
              <label class="block text-gray-700 mb-1">设备资产编码</label>
              <input type="text" id="assetCode" class="w-full h-14 px-4 border border-gray-300 rounded-lg" placeholder="例：RFH-2025">
            </div>
          \` : ''}
        \`;
      }
  ...
  return `
    <div class="p-4 pt-6 max-w-md mx-auto">
      ...
      <div id="password-prompt" class="mb-6">
        ...
        <input type="password" id="config-pwd" class="w-full h-14 px-4 border border-gray-300 rounded-lg mb-4" placeholder="默认密码：shokz@2025">
      ...
      <div id="config-form" class="hidden space-y-4">
        <div>
          <label class="block text-gray-700 mb-1">目标地址（WebSocket URL）</label>
          <input type="text" id="host-url" class="w-full h-14 px-4 border border-gray-300 rounded-lg" value="wss://mqtt-broker.shokz.com.cn:8084/mqtt">
        </div>
        <div>
          <label class="block text-gray-700 mb-1">用户名</label>
          <input type="text" id="username" class="w-full h-14 px-4 border border-gray-300 rounded-lg" value="Ml3rqYX5cPN">
        </div>
        <div>
          <label class="block text-gray-700 mb-1">密码</label>
          <input type="password" id="password" class="w-full h-14 px-4 border border-gray-300 rounded-lg" value="6jKFfGiqxjj">
        </div>
        <div>
          <label class="block text-gray-700 mb-1">Client ID</label>
          <input type="text" id="clientId" class="w-full h-14 px-4 border border-gray-300 rounded-lg" value="Ml3rqYX5cPN">
        </div>
        <div>
          <label class="block text-gray-700 mb-1">上报 Topic</label>
          <input type="text" id="topic" class="w-full h-14 px-4 border border-gray-300 rounded-lg" value="v4/p/post/thing/live/json/1.1">
        </div>
    ...
    <script>
      ...
      document.getElementById('save-btn').onclick = async () => {
        ...
        await import('./db.js').then(m => m.saveConfig(config));
        alert('✅ 配置已保存');
      };
    </script>
  `;
