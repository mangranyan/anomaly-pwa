(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))l(t);new MutationObserver(t=>{for(const n of t)if(n.type==="childList")for(const r of n.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&l(r)}).observe(document,{childList:!0,subtree:!0});function e(t){const n={};return t.integrity&&(n.integrity=t.integrity),t.referrerPolicy&&(n.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?n.credentials="include":t.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function l(t){if(t.ep)return;t.ep=!0;const n=e(t);fetch(t.href,n)}})();const x="modulepreload",h=function(a){return"/anomaly-pwa/"+a},b={},u=function(o,e,l){let t=Promise.resolve();if(e&&e.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),d=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));t=Promise.allSettled(e.map(c=>{if(c=h(c),c in b)return;b[c]=!0;const p=c.endsWith(".css"),m=p?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${m}`))return;const i=document.createElement("link");if(i.rel=p?"stylesheet":x,p||(i.as="script"),i.crossOrigin="",i.href=c,d&&i.setAttribute("nonce",d),document.head.appendChild(i),p)return new Promise((v,f)=>{i.addEventListener("load",v),i.addEventListener("error",()=>f(new Error(`Unable to preload CSS for ${c}`)))})}))}function n(r){const d=new Event("vite:preloadError",{cancelable:!0});if(d.payload=r,window.dispatchEvent(d),!d.defaultPrevented)throw r}return t.then(r=>{for(const d of r||[])d.status==="rejected"&&n(d.reason);return o().catch(n)})};let s=null;const y={"/":{render:w,bindEvents:B},"/qr-gen":{render:E,bindEvents:k},"/mqtt-config":{render:I,bindEvents:T},"/form":{render:_,bindEvents:C}};window.addEventListener("hashchange",g);g();function g(){const a=location.hash.slice(1)||"/";if(a==="/form"&&!s){location.hash="/";return}const o=y[a]||y["/"];document.getElementById("app").innerHTML=o.render(),typeof o.bindEvents=="function"&&o.bindEvents();const e=document.getElementById("bottom-nav");e&&e.classList.toggle("hidden",a==="/form")}function w(){return`
  ...
  `}async function _(){const{getDraft:a,deleteDraft:o}=await u(async()=>{const{getDraft:n,deleteDraft:r}=await import("./db-C3AG-Uqc.js");return{getDraft:n,deleteDraft:r}},[]);let e=await a();e!=null&&e.formData&&(confirm("检测到未提交草稿，是否恢复？")?s={...s,...e.formData}:await o());const l=s.type==="设备类异常",{escapeHtml:t}=await u(async()=>{const{escapeHtml:n}=await import("./utils-VTbPu1Wo.js");return{escapeHtml:n}},[]);return`
    <div class="p-4 pt-6 max-w-md mx-auto">
      <h2 class="text-xl font-bold mb-4 text-center">异常提报</h2>
      <div class="bg-gray-50 rounded-xl p-4 mb-5 space-y-3">
        <div><span class="text-gray-500 text-sm">产品</span><div class="font-medium">${t(s.product)}</div></div>
        <div><span class="text-gray-500 text-sm">产线</span><div class="font-medium">${t(s.line)}</div></div>
        <div><span class="text-gray-500 text-sm">工站</span><div class="font-medium">${t(s.station)}</div></div>
        <div><span class="text-gray-500 text-sm">异常类型</span><div class="font-medium">${t(s.type)}</div></div>
        ${l?`
          <div><span class="text-gray-500 text-sm">设备名称</span><div class="font-medium">${t(s.deviceName||"N/A")}</div></div>
          <div><span class="text-gray-500 text-sm">资产编码</span><div class="font-medium">${t(s.assetCode||"N/A")}</div></div>
        `:""}
      </div>
      <div class="space-y-4">
  ...
  `}async function B(){document.getElementById("scan-btn").onclick=async()=>{const{startScan:a}=await u(async()=>{const{startScan:o}=await import("./scanner-BnezLr2x.js");return{startScan:o}},[]);a(async o=>{const{parseQrText:e}=await u(async()=>{const{parseQrText:t}=await import("./utils-VTbPu1Wo.js");return{parseQrText:t}},[]),l=e(o);if(!l||!l.product||!l.line||!l.station||!l.type){alert("二维码格式错误：缺少必要参数");return}window.currentParams=l,location.hash="/form"})}}function k(){let a="device";function o(){const e=document.getElementById("form-fields");e.innerHTML=`
      ...
      `:""}
    `}document.getElementById("tab-device").onclick=()=>{a="device",document.getElementById("tab-device").className="flex-1 py-3 bg-blue-100 text-primary font-medium",document.getElementById("tab-process").className="flex-1 py-3 bg-gray-100 text-gray-600 font-medium",o()},document.getElementById("tab-process").onclick=()=>{a="process",document.getElementById("tab-process").className="flex-1 py-3 bg-blue-100 text-primary font-medium",document.getElementById("tab-device").className="flex-1 py-3 bg-gray-100 text-gray-600 font-medium",o()},o(),document.getElementById("gen-btn").onclick=async()=>{const e=document.getElementById("product").value.trim(),l=document.getElementById("line").value.trim(),t=document.getElementById("station").value.trim();if(!e||!l||!t){alert("请填写产品、线别、工站");return}let n=`?product=${encodeURIComponent(e)}&line=${encodeURIComponent(l)}&station=${encodeURIComponent(t)}`,r="#E8F5E9";if(a==="device"){const m=document.getElementById("deviceName").value.trim(),i=document.getElementById("assetCode").value.trim();if(!m||!i){alert("设备异常需填写设备名称和资产编码");return}n+=`&deviceName=${encodeURIComponent(m)}&assetCode=${encodeURIComponent(i)}&type=设备类异常`,r="#FFF9C4"}else n+="&type=制程类异常";const{generateQRCanvas:d}=await u(async()=>{const{generateQRCanvas:m}=await import("./qrgen-BFbIV9cV.js");return{generateQRCanvas:m}},[]),c=d(n,r),p=document.getElementById("qr-canvas-container");p.innerHTML="",p.appendChild(c),document.getElementById("qr-output").classList.remove("hidden"),document.getElementById("download-btn").onclick=()=>{c.toBlob(m=>{const i=document.createElement("a");i.href=URL.createObjectURL(m),i.download="异常二维码.png",i.click()},"image/png")}}}async function T(){document.getElementById("unlock-btn").onclick=async()=>{if(document.getElementById("config-pwd").value==="shokz@2025"){document.getElementById("password-prompt").classList.add("hidden"),document.getElementById("config-form").classList.remove("hidden");const{getConfig:o}=await u(async()=>{const{getConfig:l}=await import("./db-C3AG-Uqc.js");return{getConfig:l}},[]),e=await o();e&&(document.getElementById("host-url").value=e.hostUrl||"wss://mqtt-broker.shokz.com.cn:8084/mqtt",document.getElementById("username").value=e.username||"",document.getElementById("password").value=e.password||"",document.getElementById("clientId").value=e.clientId||"",document.getElementById("topic").value=e.topic||"")}else alert("密码错误")},document.getElementById("test-btn").onclick=async()=>{var t;const a=document.getElementById("host-url").value,o=document.getElementById("username").value,e=document.getElementById("password").value,l=document.getElementById("clientId").value;if(!a||!o||!e||!l){alert("请填写完整配置");return}try{const n=new Paho.MQTT.Client(a.replace("wss://","").split(":")[0],parseInt(a.split(":")[2]),a.split("/")[3]||"mqtt",l);await new Promise((r,d)=>{n.connect({useSSL:!0,userName:o,password:e,onSuccess:()=>{n.disconnect(),r()},onFailure:c=>d(c),timeout:5})}),alert("✅ 连接成功")}catch(n){let r="❌ 连接失败";n.errorCode===4?r="❌ 认证失败":(t=n.errorMessage)!=null&&t.includes("timeout")?r="⏱️ 连接超时":r="🌐 网络不可达",alert(r)}},document.getElementById("save-btn").onclick=async()=>{const a={hostUrl:document.getElementById("host-url").value,username:document.getElementById("username").value,password:document.getElementById("password").value,clientId:document.getElementById("clientId").value,topic:document.getElementById("topic").value},{saveConfig:o}=await u(async()=>{const{saveConfig:e}=await import("./db-C3AG-Uqc.js");return{saveConfig:e}},[]);await o(a),alert("✅ 配置已保存")}}async function C(){document.getElementById("submit-btn").onclick=async()=>{var t,n;const a=document.getElementById("reporter").value.trim(),o=document.getElementById("desc").value.trim();if(!a||!o){alert("请填写提报人和异常描述");return}const e=s.type==="设备类异常",l={product:s.product,line:s.line,workstation:s.station,deviceName:e&&s.deviceName||"",assetCode:e&&s.assetCode||"",exception_type:s.type,exception_description:o,yieldRate:e?"":((t=document.getElementById("yield"))==null?void 0:t.value.trim())||"",temporaryMeasure:e?"":((n=document.getElementById("measure"))==null?void 0:n.value.trim())||""};try{await u(()=>import("./mqtt-BZyfbBqh.js"),[]).then(r=>r.publishMessage({header:{},body:{things:[{id:"",items:[{quality:{},properties:l}]}]}})),alert("✅ 提交成功"),window.currentParams=null,setTimeout(()=>location.hash="/",3e3)}catch{try{await u(()=>import("./db-C3AG-Uqc.js"),[]).then(d=>d.saveDraft({formData:{...l,reporter:a}})),alert(`⚠️ 提交失败，请检查网络或 MQTT 配置
